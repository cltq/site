...
function getForwardedHeaders(request: Request): Record<string, string> {
  ...
  if (!headersToForward.has("user-agent")) {
    headersToForward.set("user-agent", "Mozilla/5.0 (compatible; NextJS-Proxy/1.0)");
  }
 ...
 * Aborts early if the incoming request is cancelled (client disconnected).
 */
...
export async function proxyGet(upstream: string, request: Request): Promise<Response> {
  try {
    ...
    console.log(
      `[proxyGet] Headers:`,
      Object.fromEntries(
        Object.entries(forwardedHeaders).filter(
          ([k]) => !k.toLowerCase().startsWith("authorization"),
        ),
      ),
    );
    ...
    // Client disconnected before the upstream responded - nothing to pipe anymore
    if (request.signal.aborted) {
      return new Response(null, { status: 499 });
    }
    console.error(`[proxyGet] Error:`, error);
    return new Response(
      JSON.stringify({ error: "Upstream service unavailable", details: String(error) }),
      {
        status: 502,
 ...
 */
export async function proxyWithFallback(upstreams: string[], request: Request): Promise<Response> {
