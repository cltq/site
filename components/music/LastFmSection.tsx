...
function TrackImage({ images }: { images: LastFmImage[] }) {
  ...
  if (!src) return <div className="h-12 w-12 shrink-0 rounded bg-[#171717]" />;
  return (
    <img
      src={proxyImage(src)}
      alt=""
...
function ArtistImage({ images }: { images: LastFmImage[] }) {
  const src = images?.find((i) => i["#text"])?.["#text"];
  if (!src) return <div className="h-12 w-12 shrink-0 rounded-full bg-[#171717]" />;
  return (
    <img
      src={proxyImage(src)}
      alt=""
    ...
    <div className="space-y-2">
      <AnimatePresence mode="wait">
        {tracks.map((track, i) => (
          ...
          >
            <span className="w-5 shrink-0 text-right text-sm text-[#a3a3a3]">{i + 1}</span>
            <TrackImage images={track.image} />
            <div className="min-w-0 flex-1">
              <p className="truncate text-base text-[#fafafa]">{track.name}</p>
              <p className="truncate text-sm text-[#d4d4d4]">{track.artist.name}</p>
            </div>
            <span className="shrink-0 text-sm text-[#a3a3a3]">{track.playcount}</span>
          </motion.div>
        ))}
      </AnimatePresence>
    </div>
      ...
      <AnimatePresence mode="wait">
        {artists.map((artist, i) => (
          ...
          >
            ...
            <span className="shrink-0 text-sm text-[#a3a3a3]">{artist.playcount}</span>
          </motion.div>
        ))}
      </AnimatePresence>
    </div>
...
export default function LastFmSection({ username }: { username: string }) {
  ...
  return (
    <div>
      <p className="mb-2 text-xs tracking-widest text-[#a3a3a3]">&mdash; music</p>
      <p className="mb-1 text-base text-[#d4d4d4]">songs that ive been listening</p>
      ...
      <a
        ...
        className="mt-4 inline-block text-xs text-[#a3a3a3] transition-colors duration-200 hover:text-[#d4d4d4]"
      >
        tracked via last.fm
      </a>
    </div>
