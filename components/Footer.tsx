...
export default function Footer() {
  ...
  return (
    <footer className="mx-auto w-full max-w-4xl px-8 pt-6 pb-6 pb-[calc(24px+env(safe-area-inset-bottom,0px))] text-[13px] text-[#737373]">
          ...
          >
            <span ref={innerRef} className="flex flex-col will-change-transform">
              {[...names, names[0]].map((name, i) => (
                <span
                  key={`${name}-${i}`}
                  className="whitespace-nowrap text-[#fafafa]"
